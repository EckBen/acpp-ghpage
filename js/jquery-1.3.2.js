  ...
  jQuery.fn = jQuery.prototype = {
    init: function (selector, context) {
      ...
      return this.setArray(
        jQuery.isArray(selector) ? selector : jQuery.makeArray(selector),
      );
    },
    ...
    clone: function (events) {
      ...
      var ret = this.map(function () {
        if (!jQuery.support.noCloneEvent && !jQuery.isXMLDoc(this)) {
          ...
          return jQuery.clean([
            html.replace(/ jQuery\d+="(?:\d+|null)"/g, "").replace(/^\s*/, ""),
          ])[0];
    ...
    domManip: function (args, table, callback) {
      if (this[0]) {
        var fragment = (
            this[0].ownerDocument || this[0]
          ).createDocumentFragment(),
          scripts = jQuery.clean(
            args,
            this[0].ownerDocument || this[0],
            fragment,
          ),
        ...
        if (first)
          for (var i = 0, l = this.length; i < l; i++)
            callback.call(
              root(this[i], first),
              this.length > 1 || i > 0 ? fragment.cloneNode(true) : fragment,
            );
    ...
    var Sizzle = function (selector, context, results, seed) {
      ...
      if (parts.length > 1 && origPOS.exec(selector)) {
        ...
        var ret = seed
          ...
          : Sizzle.find(
              parts.pop(),
              parts.length === 1 && context.parentNode
                ? context.parentNode
                : context,
              isXML(context),
            );
  ...
  jQuery.fn.extend({
    ...
    live: function (type, fn) {
      ...
      jQuery(document).bind(
        liveConvert(type, this.selector),
        this.selector,
        proxy,
      );
    ...
    jQuery.fn.offset = function () {
      ...
      var box = this[0].getBoundingClientRect(),
        ...
        top =
          box.top +
          (self.pageYOffset ||
            (jQuery.boxModel && docElem.scrollTop) ||
            body.scrollTop) -
          clientTop,
        left =
          box.left +
          (self.pageXOffset ||
            (jQuery.boxModel && docElem.scrollLeft) ||
            body.scrollLeft) -
          clientLeft;
